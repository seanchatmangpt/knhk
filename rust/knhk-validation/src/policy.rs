...
impl PolicyInput {
    ...
    pub fn with_run_len(mut self, len: usize) -> Self {
        self.data
            .insert("run_len".to_string(), Value::Number(len as f64));
        self
    }

    pub fn with_ticks(mut self, ticks: u32) -> Self {
        self.data
            .insert("ticks".to_string(), Value::Number(ticks as f64));
        self
    }
    ...
    pub fn with_latency_ns(mut self, latency: u64) -> Self {
        self.data
            .insert("latency_ns".to_string(), Value::Number(latency as f64));
        self
    }
    ...
    pub fn with_receipt(
        ...
        self.data.insert(
            ...
            Value::Number(timestamp_ms as f64),
        );
        self
    }
}
...
impl GuardConstraintAdvisor {
    pub fn new() -> Result<Self, PolicyError> {
        ...
        engine
            .add_policy("guard_constraints.rego", policy)
...
impl PolicyAdvisor for GuardConstraintAdvisor {
    fn evaluate(&self, input: &PolicyInput) -> Result<PolicyResult, PolicyError> {
        ...
        let result = self
            .engine
            .eval_query("data.knhk.guard_constraints.violation", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        let valid_result = self
            .engine
            .eval_query("data.knhk.guard_constraints.valid", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        if let Value::Bool(valid) = valid_result {
            if !valid && policy_result.violations.is_empty() {
                ...
                policy_result
                    .violations
                    .push("Run length validation failed".to_string());
            }
        }
...
impl PerformanceBudgetAdvisor {
    pub fn new() -> Result<Self, PolicyError> {
        ...
        engine
            .add_policy("performance_budget.rego", policy)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
...
impl PolicyAdvisor for PerformanceBudgetAdvisor {
    fn evaluate(&self, input: &PolicyInput) -> Result<PolicyResult, PolicyError> {
        ...
        let tick_result = self
            .engine
            .eval_query("data.knhk.performance_budget.violation", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        let slo_result = self
            .engine
            .eval_query("data.knhk.performance_budget.slo_violation", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        let budget_result = self
            .engine
            .eval_query("data.knhk.performance_budget.within_budget", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        if let Value::Bool(within) = budget_result {
            if !within && policy_result.violations.is_empty() {
                ...
                policy_result
                    .violations
                    .push("Performance budget exceeded".to_string());
            }
        }
...
impl ReceiptValidationAdvisor {
    pub fn new() -> Result<Self, PolicyError> {
        ...
        engine
            .add_policy("receipt_validation.rego", policy)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
...
impl PolicyAdvisor for ReceiptValidationAdvisor {
    fn evaluate(&self, input: &PolicyInput) -> Result<PolicyResult, PolicyError> {
        ...
        let result = self
            .engine
            .eval_query("data.knhk.receipt_validation.violation", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        let valid_result = self
            .engine
            .eval_query("data.knhk.receipt_validation.valid", &input_value)
            .map_err(|e| PolicyError::RegoEngineError(e.to_string()))?;
        ...
        if let Value::Bool(valid) = valid_result {
            if !valid && policy_result.violations.is_empty() {
                ...
                policy_result
                    .violations
                    .push("Receipt validation failed".to_string());
            }
        }
...
impl PolicyAdvisorChain {
    ...
    pub fn evaluate_all(&self, input: &PolicyInput) -> Vec<Result<PolicyResult, PolicyError>> {
        self.advisors
            .iter()
            .map(|advisor| advisor.evaluate(input))
            .collect()
    }
