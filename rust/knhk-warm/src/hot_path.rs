...
use crate::query::{QueryError, AskResult, SelectResult};
...
fn parse_ask_query(sparql: &str) -> Result<(u64, u64, Option<u64>), String> {
    ...
    let parts: Vec<&str> = pattern.trim().split_whitespace().collect();
    if parts.len() < 3 {
...
pub fn execute_hot_path_ask(
    graph: &WarmPathGraph,
    sparql: &str,
) -> Result<AskResult, QueryError> {
    // Parse query
    let (s, p, o) = parse_ask_query(sparql)
        .map_err(|e| QueryError::ParseError(e))?;
    ...
    if triples.len() <= 8 {
        ...
        if receipt.ticks > 8 {
            return Err(QueryError::ExecutionError(
                format!("Hot path exceeded tick budget: {} ticks", receipt.ticks)
            ));
        }
...
fn hash_iri_value(term: &oxigraph::model::Term) -> u64 {
    match term {
        oxigraph::model::Term::NamedNode(node) => {
            hash_iri(node.as_str()).unwrap_or(0)
        }
