...
fn test_invalid_sparql_syntax() {
    ...
    for invalid_query in invalid_queries {
        ...
        assert!(result.is_err(), 
               "Invalid query should return error: {}", invalid_query);
    }
}
...
fn test_very_large_query() {
    ...
    graph.load_from_turtle(&large_data).expect("Failed to load data");
    ...
    assert_eq!(select_result.bindings.len(), 100, "Should return 100 results");
}
...
fn test_concurrent_data_modification() {
    ...
    for handle in handles {
        let result = handle.join().expect("Thread should complete");
        assert!(result.is_ok(), "Query should succeed even with concurrent modifications");
    }
...
fn test_cache_eviction_behavior() {
    ...
    assert!(result.is_ok(), "Query should succeed even if evicted from cache");
}
...
fn test_query_plan_cache_behavior() {
    ...
    assert!(duration2 <= duration1 + std::time::Duration::from_millis(10),
...
fn test_large_result_set() {
    ...
    let select_result = result.unwrap();
    assert_eq!(select_result.bindings.len(), 5000, "Should return all 5000 bindings");
}
...
fn test_multiple_concurrent_graphs() {
    ...
    graph1.load_from_turtle("<s1> <p1> <o1> .").expect("Failed to load");
    graph2.load_from_turtle("<s2> <p1> <o2> .").expect("Failed to load");
    ...
    assert_ne!(select1.bindings, select2.bindings, "Graphs should have different data");
}
...
fn test_cache_consistency_under_concurrent_load() {
    ...
    for result in results {
        assert_eq!(result.bindings.len(), 1, "All queries should return same result");
    }
    
    // Check metrics
    let metrics = graph.get_metrics();
    assert!(metrics.total_queries >= 50, "Should have executed at least 50 queries");
}
