...
use knhk_warm::query::{ask_to_json, construct_to_json, select_to_json, QueryError};
...
fn test_execute_construct() {
    ...
    assert!(
        !construct_result.triples.is_empty(),
        "Should construct triples"
    );
    assert!(
        construct_result.triples.len() >= 1,
        "Should have at least 1 triple"
    );
}
...
fn test_execute_construct_empty_result() {
    ...
    assert!(
        result.is_ok(),
        "CONSTRUCT query should succeed even with no matches"
    );

    let construct_result = result.unwrap();
    assert_eq!(
        construct_result.triples.len(),
        0,
        "Should have no triples for non-matching query"
    );
}
...
fn test_execute_describe() {
    ...
    assert!(
        !describe_result.triples.is_empty(),
        "Should describe resource"
    );
    assert!(
        describe_result.triples.len() >= 2,
        "Should have at least 2 triples describing s1"
    );
}
...
fn test_execute_describe_nonexistent() {
    ...
    assert!(
        result.is_ok(),
        "DESCRIBE should succeed even for nonexistent resource"
    );

    let describe_result = result.unwrap();
    assert_eq!(
        describe_result.triples.len(),
        0,
        "Should have no triples for nonexistent resource"
    );
}
...
fn test_execute_select_empty_result() {
    ...
    assert_eq!(
        ...
        0,
        "Should have no bindings for non-matching query"
    );
    assert!(
        !select_result.variables.is_empty(),
        "Should still have variables defined"
    );
}
...
fn test_execute_ask_false() {
    ...
    assert!(
        !ask_result.result,
        "ASK query should return false for non-matching pattern"
    );
}
...
fn test_select_result_variables() {
    ...
    assert!(
        result.variables.contains(&"s".to_string()),
        "Should have variable 's'"
    );
    assert!(
        result.variables.contains(&"p".to_string()),
        "Should have variable 'p'"
    );
    assert!(
        result.variables.contains(&"o".to_string()),
