...
fn test_path_selection_accuracy() {
    ...
    assert_eq!(path, QueryPath::Hot, "Simple ASK with small data should route to hot path");
    
    // Test warm path selection
    let warm_query = "SELECT ?s ?p ?o WHERE { ?s ?p ?o }";
    let path = select_path(warm_query, 100);
    assert_eq!(path, QueryPath::Warm, "SELECT query should route to warm path");
    
    // Test warm path with larger data
    let path = select_path(warm_query, 10000);
    assert_eq!(path, QueryPath::Warm, "Should still route to warm path for ≤10K triples");
    
    // Test cold path selection (UPDATE query)
    let cold_query = "INSERT { <s> <p> <o> } WHERE {}";
    let path = select_path(cold_query, 100);
    assert_eq!(path, QueryPath::Cold, "UPDATE query should route to cold path");
    
    // Test cold path selection (data size > 10K)
    let path = select_path(warm_query, 10001);
    assert_eq!(path, QueryPath::Cold, "Large dataset should route to cold path");
}
...
fn test_path_selection_hot_path_constraints() {
    ...
    assert_ne!(path, QueryPath::Hot, "Queries with FILTER should not route to hot path");
    ...
    assert_ne!(path, QueryPath::Hot, "Queries with OPTIONAL should not route to hot path");
}
...
fn test_path_selection_warm_path_constraints() {
    ...
    assert_ne!(path, QueryPath::Warm, "UPDATE queries should not route to warm path");
    
    // Test that SHACL queries don't route to warm path
    let shacl_query = "ASK { <s> sh:hasValue <o> }";
    let path = select_path(shacl_query, 100);
    assert_ne!(path, QueryPath::Warm, "SHACL queries should not route to warm path");
}
...
fn test_concurrent_query_execution() {
    ...
    // Check metrics
    let metrics = graph.get_metrics();
    assert!(metrics.total_queries >= 10, "Should have executed at least 10 queries");
}
