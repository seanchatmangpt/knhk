...
fn test_executor_creation() {
    ...
    assert_eq!(executor.graph().size(), 0, "New executor should have empty graph");
}
...
fn test_execute_query_warm_path_select() {
    ...
    match result.unwrap() {
        QueryExecutionResult::Select(select) => {
            ...
            assert!(select.variables.contains(&"s".to_string()), "Should have variable 's'");
            assert!(select.variables.contains(&"o".to_string()), "Should have variable 'o'");
        }
...
fn test_execute_query_invalid_query() {
    ...
    assert!(error_msg.contains("parse") || error_msg.contains("failed"), 
...
fn test_executor_idempotence() {
    ...
    for _ in 0..5 {
        ...
        match result.unwrap() {
            QueryExecutionResult::Select(select) => {
                assert_eq!(select.bindings.len(), 1, "Should consistently return 1 binding");
            }
            _ => panic!("Expected Select result"),
        }
    }
...
fn test_executor_state_consistency() {
    ...
    // Load more data
    executor.load_rdf("<s2> <p1> <o2> .").expect("Failed to load data");
    ...
    match (result1, result2) {
        (QueryExecutionResult::Select(select1), QueryExecutionResult::Select(select2)) => {
            assert!(select2.bindings.len() > select1.bindings.len(),
                   "Result should reflect new data: {} vs {}", 
                   select1.bindings.len(), select2.bindings.len());
        }
