...
fn test_query_plan_cache_hit() {
    ...
    assert!(
        duration2 <= duration1 + std::time::Duration::from_millis(50),
...
fn test_query_plan_cache_miss() {
    ...
    assert!(
        metrics.total_queries >= queries.len() as u64,
...
fn test_cache_metrics_accuracy() {
    ...
    assert_eq!(
        metrics_initial.total_queries, 0,
        "Initial total queries should be 0"
    );
    assert_eq!(
        metrics_initial.cache_hits, 0,
        "Initial cache hits should be 0"
    );
    assert_eq!(
        metrics_initial.cache_misses, 0,
        "Initial cache misses should be 0"
    );
    ...
    assert_eq!(
        metrics_after_first.cache_hits, 0,
        "First query should be cache miss"
    );
    assert_eq!(
        metrics_after_first.cache_misses, 1,
    ...
    assert_eq!(
        metrics_after_second.total_queries, 2,
        "Should have 2 queries"
    );
    assert_eq!(
        metrics_after_second.cache_hits, 1,
        "Second query should be cache hit"
    );
    assert_eq!(
        metrics_after_second.cache_misses, 1,
        "Should still have 1 cache miss"
    );
    ...
    assert!(
        ...
        "Hit rate should be approximately 0.5: {}",
        hit_rate
    );
}
