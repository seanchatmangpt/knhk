...
use knhk_warm::{WarmPathConstruct8, WarmPathResult, WarmPathError};
use knhk_hot::{Ctx, Ir, Op, Run, Receipt, Engine};
...
fn test_construct8_warm_path_execution() {
    ...
    assert!(warm_result.lanes_written > 0, "Should write at least one lane");
    assert!(warm_result.latency_ms <= 500, "Should complete within 500ms");
...
fn test_construct8_warm_path_timing() {
    ...
    let s = [0x1111u64, 0x2222u64, 0x3333u64, 0x4444u64, 0x5555u64, 0x6666u64, 0x7777u64, 0x8888u64];
    let p = [0xC0FFEEu64; 8];
    let o = [0xB0Bu64; 8];
    ...
    assert!(p95_latency <= 500, "p95 latency {}ms exceeds 500ms budget", p95_latency);
