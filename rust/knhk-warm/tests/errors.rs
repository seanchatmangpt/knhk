...
use knhk_warm::{WarmPathGraph, WarmPathError};
use knhk_warm::query::{execute_select, execute_ask, execute_construct, execute_describe, QueryError};
...
fn test_query_error_execution_error() {
    ...
    assert_eq!(select_result.bindings.len(), 0, "Should return empty result");
}
...
fn test_graph_error_invalid_turtle() {
    ...
    assert!(error_msg.contains("Failed to load") || error_msg.contains("parse") || error_msg.contains("Turtle"),
           "Error should mention loading or parsing issue: {}", error_msg);
}
...
fn test_executor_error_invalid_query() {
    ...
    executor.load_rdf("<s1> <p1> <o1> .").expect("Failed to load data");
    ...
    let error_msg = result.unwrap_err();
    assert!(error_msg.contains("parse") || error_msg.contains("failed") || error_msg.contains("error"),
           "Error should mention failure: {}", error_msg);
}
...
fn test_query_error_display_implementations() {
    ...
    assert!(!unsupported_msg.is_empty(), "UnsupportedQueryType should have message");
}
...
fn test_graph_error_file_not_found() {
    ...
    let error_msg = result.unwrap_err();
    assert!(error_msg.contains("Failed to open") || error_msg.contains("No such file") || error_msg.contains("not found"),
