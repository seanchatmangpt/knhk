...
fn main() {
    ...
    println!("   Valid run (len=5): {}", if valid_result == 1 { "✅ PASS" } else { "❌ FAIL" });
    println!("   Invalid run (len=9): {}", if invalid_result == 0 { "✅ REJECTED" } else { "❌ ACCEPTED" });
    ...
    println!("   Triple 0: S={}, P={}, O={}", 
        view.subject(), view.predicate(), view.object());
    ...
    for (i, triple_view) in soa.iter_triples(5).enumerate() {
        println!("     [{}] S={}, P={}, O={}", 
            i, triple_view.subject(), triple_view.predicate(), triple_view.object());
    }
    ...
    println!("   ASK_SP({}, {}) matches ({}, {}): {}", 
        subject, predicate, target_s, target_p,
        if match_result == 1 { "✅ YES" } else { "❌ NO" });
