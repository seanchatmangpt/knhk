...
pub fn monitor_slo(class: Option<String>, window: u64) -> CnvResult<SloMonitorResult> {
    info!("Monitoring SLO compliance: class={:?}, window={} seconds", class, window);
    ...
    if data_path.exists() {
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
            if let Some(class_data) = json.get(&slo_class) {
                compliance = class_data.get("compliance")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(99.5);
                violations = class_data.get("violations")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0) as u32;
            }
        }
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
            if let Some(keys_array) = json.get("keys").and_then(|v| v.as_array()) {
                for key in keys_array {
                    ...
                    let rotation_period = key.get("rotation_period_days").and_then(|v| v.as_u64()).unwrap_or(90);
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
            if let Some(regions_array) = json.get("regions").and_then(|v| v.as_array()) {
                for region in regions_array {
                    ...
                    let data_hash = region.get("data_hash").and_then(|v| v.as_str()).unwrap_or("");
                ...
                if regions.len() > 1 {
                    let first_hash = regions[0].get("data_hash").and_then(|v| v.as_str()).unwrap_or("");
                    for region in &regions[1..] {
                        let hash = region.get("data_hash").and_then(|v| v.as_str()).unwrap_or("");
                        if hash != first_hash {
                            consistent = false;
                            break;
                        }
...
pub fn check_promotion_gate(environment: String) -> CnvResult<serde_json::Value> {
    info!("Checking promotion gate readiness for environment: {}", environment);
    ...
    criteria.insert("tests".to_string(), serde_json::json!({
        "passed": tests_passed,
        "required": true,
    }));
    if !tests_passed {
    ...
    criteria.insert("metrics".to_string(), serde_json::json!({
        "passed": metrics_ok,
        "required": true,
    }));
    if !metrics_ok {
    ...
    criteria.insert("approvals".to_string(), serde_json::json!({
        "passed": approvals_ok,
        "required": true,
    }));
    if !approvals_ok {
...
pub fn capacity_planning(model: Option<String>) -> CnvResult<serde_json::Value> {
    ...
    let current_capacity = capacity_data.get("current_capacity").and_then(|v| v.as_u64()).unwrap_or(1000);
    let utilization = capacity_data.get("utilization").and_then(|v| v.as_f64()).unwrap_or(75.0);
    let projected_demand = capacity_data.get("projected_demand").and_then(|v| v.as_u64()).unwrap_or(1200);
    let recommended_capacity = capacity_data.get("recommended_capacity").and_then(|v| v.as_u64()).unwrap_or(1500);
