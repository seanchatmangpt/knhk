...
use knhk_etl::{IngestStage, PipelineError, RawTriple};
...
fn test_construct8_pipeline_rust_to_c_to_rust() {
    ...
    let raw_triples = ingest
        .parse_rdf_turtle(turtle_content)
    ...
    println!(
        "✓ Prepared CONSTRUCT8 IR: pred=0x{:x}, len={}",
        pred, run.len
    );
    ...
    println!(
        "✓ C hot path executed: {} triples, {:.2}ms",
        written, latency_ms
    );
    ...
    assert!(
        latency_ms < 500.0,
        "CONSTRUCT8 should complete in warm path budget (<500ms), got {:.2}ms",
        latency_ms
    );

    println!(
        "✓ Pipeline complete: {} triples, {:.2}ms, span_id=0x{:x}",
...
fn test_construct8_pipeline_performance() {
    ...
    println!(
        "✓ Performance test: max_ticks={}, max_ns={:.2}",
        max_ticks, max_ns
    );
    ...
    if max_ticks > 8 {
        println!(
            "  ⚠ Performance gap: max_ticks={} exceeds budget=8 (known issue)",
            max_ticks
        );
