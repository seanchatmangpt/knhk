...
pub fn get() -> Result<std::collections::HashMap<String, String>, String> {
    ...
    let _span = span!(
        Level::INFO,
        "knhk.metrics.get",
        knhk.operation.name = "metrics.get"
    );
    ...
    metrics.insert(
        "hook_latency_p50".to_string(),
        format!("{:.1}", storage.hook_latency_p50),
    );
    metrics.insert(
        "hook_latency_p95".to_string(),
        format!("{:.1}", storage.hook_latency_p95),
    );
    metrics.insert(
        "drift_violations".to_string(),
        storage.drift_violations.to_string(),
    );
    metrics.insert(
        "connector_throughput".to_string(),
        storage.connector_throughput.to_string(),
    );
    metrics.insert(
        "receipt_generation_rate".to_string(),
        storage.receipt_generation_rate.to_string(),
    );
...
pub fn weaver_start(
    ...
    let _span = span!(
        ...
        knhk.operation.name = "weaver.start"
    );
    let _enter = _span.enter();
...
pub fn weaver_stop(admin_port: Option<u16>) -> Result<(), String> {
    let _span = span!(
        ...
        knhk.operation.name = "weaver.stop"
    );
    let _enter = _span.enter();
...
pub fn weaver_validate(
    ...
    let _span = span!(
        ...
        knhk.operation.name = "weaver.validate"
    );
    let _enter = _span.enter();

    // Start Weaver with output directory for reports
    let temp_output =
        std::env::temp_dir().join(format!("weaver-validation-{}", std::process::id()));
    ...
    tracer
        .export_to_weaver(&format!("http://{}/v1/traces", endpoint))
...
fn parse_weaver_report(output_dir: &std::path::Path) -> Result<ValidationResult, String> {
    ...
    let latest_report = report_files
        .iter()
