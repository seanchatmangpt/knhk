...
fn get(id: String) -> Result<ReceiptResult> {
    let receipt = receipt_impl::get(id.clone())
        .map_err(|e| clap_noun_verb::NounVerbError::execution_error(format!("Failed to get receipt: {}", e)))?;
...
fn merge(ids: String) -> Result<MergeResult> {
    let receipt = receipt_impl::merge(ids.clone())
        .map_err(|e| clap_noun_verb::NounVerbError::execution_error(format!("Failed to merge receipts: {}", e)))?;
...
fn list() -> Result<ReceiptList> {
    receipt_impl::list()
        .map_err(|e| clap_noun_verb::NounVerbError::execution_error(format!("Failed to list receipts: {}", e)))
...
fn verify(id: String) -> Result<VerifyResult> {
    let valid = receipt_impl::verify(id.clone())
        .map_err(|e| clap_noun_verb::NounVerbError::execution_error(format!("Failed to verify receipt: {}", e)))?;
...
fn show(id: String) -> Result<ShowReceiptResult> {
    let receipt = receipt_impl::show(id.clone())
        .map_err(|e| clap_noun_verb::NounVerbError::execution_error(format!("Failed to show receipt: {}", e)))?;
