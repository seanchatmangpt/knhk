...
async fn test_circuit_breaker_initial_state() {
    ...
    let cb = SidecarCircuitBreaker::new("http://test:8080".to_string(), 5, 1000);
...
async fn test_circuit_breaker_success_resets_failure_count() {
    ...
    let cb = SidecarCircuitBreaker::new("http://test:8080".to_string(), 3, 1000);
...
async fn test_circuit_breaker_registry() {
    ...
    let cb1 = registry
        .get_or_create("http://service1:8080".to_string())
        .expect("Should get or create circuit breaker");
    let cb2 = registry
        .get_or_create("http://service2:8080".to_string())
        .expect("Should get or create circuit breaker");
...
async fn test_health_checker_check_interval() {
    ...
    assert!(
        second_check < Duration::from_millis(50),
...
async fn test_metrics_collector_records_latency() {
    ...
    assert!(
        snapshot.latency.p95_ms >= snapshot.latency.p50_ms,
        "p95 should be >= p50"
    );
    assert!(
        snapshot.latency.p99_ms >= snapshot.latency.p95_ms,
        "p99 should be >= p95"
    );
}
...
async fn test_metrics_collector_reset() {
    ...
    assert_eq!(
        snapshot_after.requests.total, 0,
        "Should reset to 0 requests"
    );
}
...
fn test_sidecar_config_defaults() {
    ...
    assert!(
        ...
        "Retry max attempts should be > 0"
    );
}
...
fn test_sidecar_error_display() {
    ...
    assert!(
        error_str.contains("Connection failed") || error_str.contains("NetworkError"),
...
async fn test_sidecar_client_config() {
    ...
    assert!(
        !config.warm_orchestrator_url.is_empty(),
...
fn test_tls_config_structure() {
    ...
    assert_eq!(
        config.cert_file,
        Some("test.crt".to_string()),
        "Cert file should be set"
    );
    assert_eq!(
        config.key_file,
        Some("test.key".to_string()),
        "Key file should be set"
    );
    assert_eq!(
        config.ca_file,
        Some("ca.crt".to_string()),
        "CA file should be set"
    );
}
...
async fn test_metrics_provide_observability() {
    ...
    assert!(
        snapshot.requests.success > 0,
        "Should track successful requests"
    );
    assert!(
        snapshot.requests.failure > 0,
...
async fn test_health_checks_enable_monitoring() {
    ...
    // Initial state
    let status = checker.status();
    assert_eq!(status, HealthStatus::Healthy, "Should start healthy");
    ...
    checker.set_healthy();
    let status = checker.status();
    assert_eq!(status, HealthStatus::Healthy, "Should support recovery");
}
