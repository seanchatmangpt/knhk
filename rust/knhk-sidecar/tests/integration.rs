...
mod tests {
    ...
    async fn test_retry_executor() {
        use knhk_sidecar::retry::{RetryConfig, RetryExecutor};
        ...
        let result = executor
            .execute(|| async { Ok::<String, SidecarError>("success".to_string()) })
            .await;
    ...
    #[test]
    fn test_error_types() {
        use knhk_sidecar::error::{is_guard_violation, is_retryable_error, SidecarError};
    ...
    fn test_config_loading() {
        ...
        assert_eq!(
            config.client.warm_orchestrator_url,
