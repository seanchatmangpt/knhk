        ...
        if let oxigraph::sparql::QueryResults::Solutions(solutions) = results {
            for solution_result in solutions {
                let solution = solution_result
                    .map_err(|e| WorkflowError::Internal(format!("Query solution error: {:?}", e)))?;
                
                let mut row = HashMap::new();
                for var in solution.variables() {
                    if let Some(term) = solution.get(var) {
                        let value = match term {
                            oxigraph::model::Term::NamedNode(node) => Value::String(node.to_string()),
                            oxigraph::model::Term::BlankNode(node) => Value::String(format!("_:{}", node)),
...
"#;
        ...
        assert!(result.is_ok() || result.is_err(), "Generation should complete");
    }
}
