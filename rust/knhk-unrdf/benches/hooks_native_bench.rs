...
fn benchmark_batch_hook_evaluation(c: &mut Criterion) {
    ...
    for batch_size in [1, 10, 100, 1000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, hooks| {
                b.iter(|| {
                    black_box(evaluate_hooks_batch_native(black_box(hooks), black_box(&turtle_data))).unwrap()
                });
            },
        );
    }
...
fn benchmark_hook_receipt_generation(c: &mut Criterion) {
    ...
    group.bench_function("generate_receipt", |b| {
        b.iter(|| {
            let result = black_box(evaluate_hook_native(black_box(&hook), black_box(&turtle_data))).unwrap();
...
fn benchmark_query_complexity(c: &mut Criterion) {
    ...
    for (name, query) in queries {
        ...
        group.bench_function(name, |b| {
            b.iter(|| {
                black_box(evaluate_hook_native(black_box(&hook), black_box(&turtle_data))).unwrap();
            });
        });
    }
