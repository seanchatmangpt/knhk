...
pub use query::{detect_query_type, query_sparql, query_sparql_with_data, query_sparql_with_type};
...
pub use types::{
    HookDefinition, HookResult, QueryResult, RdfFormat, ShaclValidationResult, ShaclViolation,
    SparqlQueryType, TransactionReceipt,
};
...
pub fn init_unrdf(unrdf_path: &str) -> Result<(), UnrdfError> {
    ...
    if !path.exists() {
        return Err(UnrdfError::InitializationFailed(format!(
            "unrdf directory does not exist: {}",
            unrdf_path
        )));
    }
    if !path.is_dir() {
        return Err(UnrdfError::InitializationFailed(format!(
            "unrdf path is not a directory: {}",
            unrdf_path
        )));
    }
    ...
    if !knowledge_engine.exists() {
        return Err(UnrdfError::InitializationFailed(format!(
            "unrdf knowledge engine not found at: {}",
            knowledge_engine.display()
        )));
    }

    let runtime = Runtime::new().map_err(|e| {
        UnrdfError::InitializationFailed(format!("Failed to create runtime: {}", e))
    })?;
