...
impl TemplateEngine {
    ...
    fn new() -> Result<Self, UnrdfError> {
        ...
        tera.add_raw_template("query-with-data", include_str!("../templates/query-with-data.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load query-with-data template: {}", e)))?;
        
        tera.add_raw_template("query-only", include_str!("../templates/query-only.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load query-only template: {}", e)))?;
        
        tera.add_raw_template("hook-execute", include_str!("../templates/hook-execute.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load hook-execute template: {}", e)))?;
        
        tera.add_raw_template("hook-execute-with-data", include_str!("../templates/hook-execute-with-data.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load hook-execute-with-data template: {}", e)))?;
        
        tera.add_raw_template("hook-register", include_str!("../templates/hook-register.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load hook-register template: {}", e)))?;
        
        tera.add_raw_template("store", include_str!("../templates/store.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load store template: {}", e)))?;
        
        tera.add_raw_template("transaction-commit", include_str!("../templates/transaction-commit.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load transaction-commit template: {}", e)))?;
        
        tera.add_raw_template("shacl-validate", include_str!("../templates/shacl-validate.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load shacl-validate template: {}", e)))?;
        
        tera.add_raw_template("serialize", include_str!("../templates/serialize.tera"))
            .map_err(|e| UnrdfError::InvalidInput(format!("Failed to load serialize template: {}", e)))?;
    ...
    fn escape_js_filter(value: &Value, _args: &HashMap<String, Value>) -> TeraResult<Value> {
        let s = value.as_str().ok_or_else(|| tera::Error::msg("escape_js filter requires a string value"))?;
        Ok(Value::String(
            s.replace('\\', "\\\\")
             .replace('`', "\\`")
             .replace('$', "\\$")
