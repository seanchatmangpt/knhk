...
pub fn execute_hook(hook_name: &str, hook_query: &str) -> UnrdfResult<HookResult> {
    ...
    let script = template_engine
        ...
        .map_err(|e| {
            UnrdfError::InvalidInput(format!("Failed to render hook-execute template: {}", e))
        })?;
    ...
    state.runtime.block_on(async {
        ...
        let json_line = output
            ...
            .ok_or_else(|| {
                UnrdfError::HookFailed(format!("No JSON found in output. Full output: {}", output))
            })?;

        let result: HookResult = serde_json::from_str(json_line.trim()).map_err(|e| {
            UnrdfError::HookFailed(format!(
    ...
    let script = template_engine
        ...
        .map_err(|e| {
            UnrdfError::InvalidInput(format!(
                "Failed to render hook-execute-with-data template: {}",
                e
            ))
        })?;
...
pub fn register_hook(hook_json: &str) -> UnrdfResult<String> {
    ...
    let script = template_engine
        ...
        .map_err(|e| {
            UnrdfError::InvalidInput(format!("Failed to render hook-register template: {}", e))
        })?;
...
pub fn list_hooks() -> UnrdfResult<Vec<HookDefinition>> {
    ...
    let hook_list: Vec<HookDefinition> = hooks
        .values()
