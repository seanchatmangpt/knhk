...
pub fn begin_transaction(actor: &str) -> UnrdfResult<u32> {
    ...
    let mut next_id = state.next_transaction_id.lock().map_err(|e| {
        UnrdfError::InvalidInput(format!("Failed to acquire transaction ID lock: {}", e))
    })?;

    let transaction_id = *next_id;
    *next_id += 1;

    let mut transactions = state.transactions.lock().map_err(|e| {
        UnrdfError::InvalidInput(format!("Failed to acquire transactions lock: {}", e))
    })?;
...
pub fn transaction_add(transaction_id: u32, turtle_data: &str) -> UnrdfResult<()> {
    ...
    let transaction = transactions.get_mut(&transaction_id).ok_or_else(|| {
        UnrdfError::InvalidInput(format!("Transaction {} not found", transaction_id))
    })?;
    ...
    match transaction.state {
        ...
        _ => Err(UnrdfError::InvalidInput(format!(
...
pub fn commit_transaction(transaction_id: u32) -> UnrdfResult<TransactionReceipt> {
    ...
    match transaction.state {
        TransactionState::Pending => {
            ...
            context.insert(
                "additions_json",
                &serde_json::to_string(&additions).map_err(|e| {
                    UnrdfError::InvalidInput(format!("Failed to serialize additions: {}", e))
                })?,
            );
            context.insert(
                "removals_json",
                &serde_json::to_string(&removals).map_err(|e| {
                    UnrdfError::InvalidInput(format!("Failed to serialize removals: {}", e))
                })?,
            );
            ...
            let script = template_engine
                ...
                .map_err(|e| {
                    UnrdfError::InvalidInput(format!(
                        "Failed to acquire template engine lock: {}",
                        e
                    ))
                })?
                ...
                .map_err(|e| {
                    UnrdfError::InvalidInput(format!(
                        "Failed to render transaction-commit template: {}",
                        e
                    ))
                })?;
            ...
            let receipt_result = state.runtime.block_on(async {
                ...
                let receipt: TransactionReceipt = serde_json::from_str(&output).map_err(|e| {
                    UnrdfError::QueryFailed(format!(
                        "Failed to parse receipt: {} - output: {}",
                        e, output
                    ))
                })?;
                Ok(receipt)
