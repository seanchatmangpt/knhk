...
// Allow not_unsafe_ptr_arg_deref - FFI functions intentionally take raw pointers
// The unsafe contract is documented in each function and enforced by C callers
#![allow(clippy::not_unsafe_ptr_arg_deref)]

use crate::error::UnrdfError;
use crate::hooks::{
    deregister_hook, execute_hook, execute_hook_with_data, list_hooks, register_hook,
};
...
use crate::transaction::{
    begin_transaction, commit_transaction, rollback_transaction, transaction_add,
    transaction_remove,
};
...
pub extern "C" fn knhk_unrdf_store_turtle(turtle_data: *const c_char) -> c_int {
    ...
    match data {
        Ok(d) => match store_turtle_data(d) {
            Ok(_) => 0,
            Err(_) => -1,
        },
        Err(_) => -1,
...
pub extern "C" fn knhk_unrdf_query(
    ...
    match q {
        Ok(query_str) => match query_sparql(query_str) {
...
pub extern "C" fn knhk_unrdf_execute_hook(
    ...
    match (name, query) {
        (Ok(n), Ok(q)) => match execute_hook(n, q) {
            Ok(result) => match serde_json::to_string(&result) {
                Ok(json) => {
                    let json_bytes = json.as_bytes();
                    if json_bytes.len() < result_size {
...
pub extern "C" fn knhk_unrdf_execute_hook_with_data(
    ...
    if !turtle_data.is_null() {
        ...
        match (name, query, data) {
            (Ok(n), Ok(q), Ok(d)) => match execute_hook_with_data(n, q, d) {
                Ok(result) => match serde_json::to_string(&result) {
                    Ok(json) => {
                        let json_bytes = json.as_bytes();
                        if json_bytes.len() < result_size {
...
pub extern "C" fn knhk_unrdf_transaction_begin(actor: *const c_char) -> c_int {
    ...
    match actor_str {
        Ok(a) => match begin_transaction(a) {
            Ok(id) => id as c_int,
            Err(_) => -1,
        },
        Err(_) => -1,
...
#[no_mangle]
pub extern "C" fn knhk_unrdf_transaction_add(
    ...
    match data {
        Ok(d) => match transaction_add(transaction_id as u32, d) {
            Ok(_) => 0,
            Err(_) => -1,
        },
        Err(_) => -1,
...
#[no_mangle]
pub extern "C" fn knhk_unrdf_transaction_remove(
    transaction_id: c_int,
    turtle_data: *const c_char,
) -> c_int {
    let data = unsafe {
    ...
    match data {
        Ok(d) => match transaction_remove(transaction_id as u32, d) {
            Ok(_) => 0,
            Err(_) => -1,
        },
        Err(_) => -1,
...
pub extern "C" fn knhk_unrdf_validate_shacl(
    ...
    match (data, shapes) {
        (Ok(d), Ok(s)) => match validate_shacl(d, s) {
            Ok(result) => match serde_json::to_string(&result) {
                Ok(json) => {
                    let json_bytes = json.as_bytes();
                    if json_bytes.len() < result_size {
...
pub extern "C" fn knhk_unrdf_register_hook(
    ...
    match json_str {
        Ok(json) => match register_hook(json) {
            Ok(id) => {
...
pub extern "C" fn knhk_unrdf_list_hooks(hooks_json: *mut c_char, hooks_size: usize) -> c_int {
    match list_hooks() {
        Ok(hooks) => match serde_json::to_string(&hooks) {
            Ok(json) => {
                let json_bytes = json.as_bytes();
