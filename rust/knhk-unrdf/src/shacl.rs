...
pub fn validate_shacl(data_turtle: &str, shapes_turtle: &str) -> UnrdfResult<ShaclValidationResult> {
    ...
    let script = template_engine.lock()
        ...
        .map_err(|e| UnrdfError::InvalidInput(format!("Failed to render shacl-validate template: {}", e)))?;
    ...
    state.runtime.block_on(async {
        ...
        let result: ShaclValidationResult = serde_json::from_str(json_line.trim())
